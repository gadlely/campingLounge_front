...
function AdminSignIn() {
    ...
    const emailDuplicate = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/member/emailDuplicate`, { email });
            if (!response.data) {
                setIsValid(true);
                alert("사용 가능한 이메일입니다.");
            } else {
    ...
    const checkKey = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/member/checkKey`, data);
